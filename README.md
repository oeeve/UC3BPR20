...
The following IoT / smart home forensics <a href="#dataset">dataset</a> is a result of a digital forensics experiment, based on a wireless smart home testbed consisting of an Atom Homey Pro smart home gateway and a set of Zigbee sensors and actuators, managed by the Homey Android application emulated on an Android Studio AVD. The resulting forensic artefacts (in the form of forensic images of the Homey Pro's eMMC storage, as well as logically extracted Android data, before and after execution of the experiment) has been made available in this <a href="#dataset">dataset</a>, which aims to support future research, training, and testing in IoT forensics (also feel free to use it in CTF challenges).
...
1. Are you able to identify the type of OS and the startup processes?.
2. Where is the Homey software located and what technologies are used?.
3. Where are the site- and user-specific configurations and logs stored?.
4. Can you find any correlations between the user/site data located in the eMMC and the Android application?.
...
| --- | ---------------------- |
| Atom Homey Pro | Smart home hub and centre of the WPAN Zigbee network. The network's sensors and actuators are paired to this unit, which in turn is managed by the Homey app. `Model: HY0025`. |
...
| Ikea Badring | Wireless Zigbee water leak sensor. `Item No. 605.043.52`. |
...
| 5  | Two 'flows' were configured, turning lights on/off on motion/no motion in the office. |
| 6  | Account Name: `UC3 BPR` *|
| 7  | Wi-Fi Credentials: `Alti***443108`, `pqVPvReK` *                                     |
| 8  | Account Credentials: `UC3BPR20@***web.io`, `helloUC3BPR20` *|

*All credentials used in the testbed were changed/closed after the end of the experiment.
...
| `/var/lib/docker/overlay2/ | Docker’s copy-on-write storage area where the image layer store changes.        |
...
| Location        | Significance |
| --------------- | ------------ |
| `/sqlite/db.sqlite` | Main database, containing information on the user account, smart home site and configurations, sensors paired, their configurations and parts of their logged data, cloud keys, timestamps, and more. |
| `/network-manager-connections` | Wi-Fi credentials, including password in plaintext. |
| `log/system.log` | System events, such as the start of the Homey Pro Docker container. |
| `log/homey-pro.log` | Homey Pro application events such as network adapters and cloud manager. |
| `/apps/}` | Directory for third-party plugins for Homey Pro, such as Ikea Tradfri and Eva HAN-sensor. |
...
<figure>
  ...
  <figcaption>Figure 3: Dockers Overlay2 storage driver - File system "layers".</figcaption>
</figure>
<p>&nbsp;</p>

<figure>
  <img src="media/5_homey_3_snooper.webp" alt="Homey Pro">
  <figcaption>Figure 4: Configuration file for UARTs (enable/disable) + message from the Homey developers.</figcaption>
</figure>
<p>&nbsp;</p>
...
| Location | Significance |
| -------- | ------------ |
| `/data/data/app.homey` | Primary application folder (see directory tree below). |
...
1. Physically extract the CM4 SoM from the Homey Pro's main PCB.
2. Configure and mount the CM4 in the [CM4IO](https://www.raspberrypi.com/products/compute-module-4-io-board/) development board.
3. Mount the eMMC as a USB storage device on a host computer, using [`rpiboot`](https://github.com/raspberrypi/usbboot).
4. 'bit-for-bit' clone the eMMC to a raw .img file using `dd`.
5. Create a forensic image (EnCase EWF / .E01) of the raw file.
...
Enable write protection with `sudo blockdev --setro /dev/sda` (given `sda` is identified as the eMMC). Then verify that it has been enabled by running `blockdev --getro /dev/sda`, where `1` is enabled, and `0` is disabled. 
...
#### `dd`
Example of use of `dd`. Make sure to double check disk device name with `lsblk`.
```
